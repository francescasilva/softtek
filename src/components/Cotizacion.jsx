      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="max-w-xl mx-auto text-center space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 justify-center">
            {/* Opción 1: Para mí */}
            <label
              className={`group relative rounded-xl p-6 bg-white w-full max-w-xs shadow-xl transform transition duration-300 hover:-translate-y-1 cursor-pointer border-0 ${
                opcionSeleccionada === 'para-mi' ? 'border-blue-500 ring-2 ring-blue-200' : ''
              }`}
              onClick={() => setOpcionSeleccionada('para-mi')}
            >
              <input
                type="radio"
                name="option"
        ...
        {opcionSeleccionada && (
          <div className="flex justify-center mt-10">
            <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-10 justify-items-center ">
              {planesFiltrados.length > 0 ? (
                planesFiltrados.map((plan, index) => (
                  <div
                    key={index}
                    className="flex flex-col justify-between rounded-2xl border border-gray-200 shadow-md p-12 bg-white w-full max-w-xs min-h-[640px]"
                  >
                    ...
                    <button
                      ...
                      className="mt-auto bg-red-500 text-white text-lg font-semibold py-2 px-5 rounded-4xl hover:bg-red-600 transition duration-200"
                    >
                      Seleccionar Plan
                    </button>
                  </div>
